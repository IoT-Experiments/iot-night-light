...
String createJwt(String project_id, long long int time, NN_DIGIT *priv_key, unsigned int tokenLifetime) {
 ecc_init();
 // Making jwt token json
 String header = "{\"alg\":\"ES256\",\"typ\":\"JWT\"}";
 String payload = "{\"iat\":" + int_to_string(time) +
                  ",\"exp\":" + int_to_string(time + tokenLifetime) + ",\"aud\":\"" +
                  project_id.c_str() + "\"}";
 ...
 return String((header_payload_base64 + "." +
